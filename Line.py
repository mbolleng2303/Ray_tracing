...
class Line:
    def __init__(self, StartVec, EndVec):
        self.Vec1 = np.array([round(StartVec[0],5),round(StartVec[1],5)])
        self.Vec2 = np.array([round(EndVec[0],5),round(EndVec[1],5)])

        self.Direction = self.Vec1 - self.Vec2


        self.Distance = round(np.linalg.norm(self.Direction),5)
        #print(self.Distance)
        #print(self.Direction, self.Distance)

        self.Direction = np.array([round(self.Direction[0] / self.Distance , 5),round(self.Direction[1]/ self.Distance,5)])
    ...
    def MidPoint(self):
        res = self.Vec2 + self.Direction * (self.Distance / 2)
        return res
    ...
    def Intersect(self, OtherLine: "Line"):
        ...
        num = (OtherLine.Direction[1] * (self.Vec1[0] - OtherLine.Vec1[0])) - (
                    OtherLine.Direction[0] * (self.Vec1[1] - OtherLine.Vec1[1]))
...
class Line:
    ...
    def Compute_Theta_I(self, OtherLine):
        """ Is a method of class "Line" wish is instanced during the image method when we want to compute
        the incidence angle between the ray ( Line object : "self") and
         the normal vector of the wall( argument : "Other Line")
       """
        angle = np.arccos(np.dot(self.Direction, OtherLine.Direction))
